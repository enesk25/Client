...
const long c_lNormal_Waiting_Time = 3;//@fixme255
...
void CPythonEventManager::ProcessEventSet(TEventSet* pEventSet)
{
	...
	case EVENT_TYPE_QUEST_BUTTON:
	{
		...
		if (!strcmp(name, "조각난 용혼석?....")) {
			PyCallClassMemberFunc(m_poGame, "BINARY_RecvQuest", Py_BuildValue("(isss)", idx, name, "highlight", "")); //@fixme307
		}
		else {
			if (c_rstFile.empty())
			{
				PyCallClassMemberFunc(m_poGame, "RecvQuest", Py_BuildValue("(is)", idx, name)); //@fixme307
			}
			else
			{
				PyCallClassMemberFunc(m_poGame, "BINARY_RecvQuest",
					Py_BuildValue("(isss)", idx, name, c_rstType.c_str(), c_rstFile.c_str())); //@fixme307
			}
		}
		break;
	}
