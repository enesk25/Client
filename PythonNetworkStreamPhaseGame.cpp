...
void CPythonNetworkStream::GamePhase()
{
	...
	while (ret)
	{
...
#ifdef ENABLE_MINIMAP_PARTY_POSITION
		...
		case HEADER_GC_UNK_213: // @fixme328
			ret = RecvUnk213();
			break;
...
bool CPythonNetworkStream::RecvUnk213() // @fixme328
{
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			if (pInstTarget->IsPC() || pInstTarget->IsBuilding())
			{
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetPCTargetBoard", Py_BuildValue("(isiLL)", TargetPacket.dwVID, pInstTarget->GetNameString(), TargetPacket.bHPPercent, TargetPacket.iMinHP, TargetPacket.iMaxHP));
#else
...

{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_ITEM_SHOP_DATA", Py_BuildValue("(iiiiiiii)", p.id, p.category, p.sub_category, p.vnum, p.count, p.coinsold,p.coins,p.socketzero));
	return true;
}
#endif
