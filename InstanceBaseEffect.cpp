...
void CInstanceBase::AttachTextTail()
{
	...
	float fTextTailHeight = IsMountingHorse() ? 110.0f : GetBaseHeight() + 10.0f;
#else
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	static D3DXCOLOR s_kPlayerLevelColor = D3DXCOLOR(191.0f / 255.0f, 217.0f / 255.0f, 255.0f / 255.0f, 1.0f);
#else
...
void CInstanceBase::__SetAffect(UINT eAffect, bool isVisible)
{
	...
	case AFFECT_INVISIBILITY:
		...
		if (isVisible)
		{
			m_GraphicThingInstance.HideAllAttachingEffect(); //@fixme288
		...
		{
			m_GraphicThingInstance.BlendAlphaValue(1.0f, 1.0f);
			m_GraphicThingInstance.ShowAllAttachingEffect(); //@fixme288
			AttachTextTail();
			RefreshTextTail();
		}
		return;
...
bool CInstanceBase::IsPossibleEmoticon()
{
	...
	{
#ifndef ENABLE_HIDE_EMOTION_LOG
