...
const float c_fDefaultHorseRotationSpeed = 1500.0f; //@fixme253
...
BOOL CInstanceBase::IsPet()// @fixme237 //pet vnumlarý yazýlacak..
{
...
BOOL CInstanceBase::IsMount()// @fixme237 //mount vnumlarý yazýlacak..
{
	...
	case CItemData::ITEM_TYPE_COSTUME:
		...
		if (pItem->GetSubType() == CItemData::COSTUME_WEAPON)
		{
			__ClearWeaponRefineEffect();
				...
				if (40101 == vnum) // KILIÇ
				{
					m_swordRefineEffectRight = EFFECT_REFINED+EFFECT_100;
					{
						if (m_swordRefineEffectRight)
							m_swordRefineEffectRight = __AttachEffect(m_swordRefineEffectRight);
					}
				...
				if (40102 == vnum) // HANÇER
				{
					m_swordRefineEffectRight = EFFECT_REFINED+EFFECT_101;
					m_swordRefineEffectLeft = EFFECT_REFINED+EFFECT_102;
					{
					if (m_swordRefineEffectRight)
						m_swordRefineEffectRight = __AttachEffect(m_swordRefineEffectRight);
					if (m_swordRefineEffectLeft)
				...
				if (40103 == vnum) // YAY
				{
					m_swordRefineEffectRight = EFFECT_REFINED+EFFECT_103;
					{
						if (m_swordRefineEffectRight)
							m_swordRefineEffectRight = __AttachEffect(m_swordRefineEffectRight);
					}
				...
				if (40105 == vnum) //ÇAN
				{
					m_swordRefineEffectRight = EFFECT_REFINED+EFFECT_104;
					{
						if (m_swordRefineEffectRight)
							m_swordRefineEffectRight = __AttachEffect(m_swordRefineEffectRight);
					}
...
#ifdef ZIRH_EFEKT
			DWORD vnum = pItem->GetIndex();
			if (55076 <= vnum && vnum <= 55077)
			{
				__AttachEffect(EFFECT_REFINED + EFFECT_COSTUME_TWITCH);
			}
			if (55078 <= vnum && vnum <= 55079)
			{
