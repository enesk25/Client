...
PyObject* playerKeyAutoAttack (PyObject* poSelf, PyObject* poArgs)
{
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	DWORD TargetVID = CPythonPlayer::Instance().GetTargetVID();
	rkPlayer.Key_AutoAttack (TargetVID);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_COINS_RENEWAL_SYSTEM
...
PyObject* playerGetItemLink(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		{
			// if (pPlayerItem->aAttr[i].bType != 0) // @fixme230 (this line must be commented)
			{
...
PyObject* playerIsValuableItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 2:
		if (!PyTuple_GetInteger(poArgs, 0, &SlotIndex.window_type))
			return Py_BuildException();
		if (!PyTuple_GetInteger(poArgs, 1, &SlotIndex.cell)) // @fixme232 arg idx from 0 to 1
			return Py_BuildException();
		break;
	default:
		return Py_BuildException();
...
void initPlayer()
{
...
#ifdef ENABLE_NEW_FOLLOW_AND_ATTACK
		{ "KeyAutoAttack", playerKeyAutoAttack, METH_VARARGS},
#endif
	...
	PyModule_AddIntConstant(poModule, "POINT_ATT_BONUS", POINT_ATT_BONUS); //@fixme128
	...
	PyModule_AddIntConstant(poModule, "EMOTION_BREAKDANCE", EMOTION_BREAKDANCE);
#endif
